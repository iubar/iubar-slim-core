...
abstract class JsonSafeAbstractController extends JsonAbstractController {
    ...
    private function sendHmacError($debug = false){
        $request = $this->app->request;
        $user = $request->params('user');
        $ts_str = rawurldecode($request->params('ts'));
        ...
        if(!$debug){
            ...
            if(!$user || !$ts_str || !$hash){
                ...
                $error .= "\$user = $user";
                $error .= "\$hash = $hash";
                $error .= "\$allPostVars = " . json_encode($allPostVars);
                $code = ResponseCode::BAD_REQUEST;
            }else if(!$ts){
                ...
                $expected_hash = $this->calcHash($user, $api_key, $ts_str); // WARNING: security breach !!!
    ...
    protected function validate(){
        $user = $this->app->request->params('user');
        ...
        $ts_str = rawurldecode($this->app->request->params('ts'));
