...
abstract class ApiJwtController extends JsonAbstractController {
			...
			$jwt =  $request->params('jwt');
			 ...
			 if ($jwt) {
			 	try {
			 		...
			 		$token = JWT::decode($jwt, $secretKey, array($this->algorithm));
			 		print_r($decoded);
			 		/*
	...
	protected function getJwtToken($email{
		...
		if ($this->isUserValid($email, $api_key)){
	...
	private function isUserValid($email, $api_key){
		return $this->isUserRegistered($email);
	}
